...
def run_tests(module_path, pyver, source_folder, tmp_folder, flavor, excluded_tags, include_tags,
    ...
    #  --nocapture
    venv = "virtualenv" if platform.system() != "Darwin" else f"{pyenv} -m virtualenv"
    command = f"{venv} --python \"{pyenv}\" \"{venv_dest}\" && " \
              f"{source_cmd} \"{venv_exe}\" && " \
              f"{pip_installs} " \
              f"python setup.py install && " \
              f"conan --version && conan --help && " \
              f"pytest -rf {module_path} {tags_str} " \
              f"{multiprocess} --durations=20"
